...
function Sale() {
  ...
  const touchMessages = ["Ohhh I got touched by a Queen! 👑✨","Your Majesty touched me! 💕","A Queen's touch! I'm blessed! 🌟","Royal vibes detected! 👑💖","Feeling royal now! ✨👑","Queen energy is real! 💅✨"];
  const addToCartMessages = ["Ohhh nice choice Beautiful! 💎","Yasss Queen! Perfect pick! 👑","You've got amazing taste! ✨","That's gonna look stunning on you! 💕","Absolutely gorgeous choice! 🌟","Your style is impeccable! 💖","This piece was made for you! ✨","Can't wait to see you shine! 💎"];
  const hoverMessages = ["Ooh, exploring? Love it! 👀✨","That one's a beauty! 💎","Great eye, gorgeous! 👑","You're gonna look amazing! 💕","This collection is fire! 🔥","Treat yourself, Queen! 💅"];
  ...
  const handleCharacterClick = () => showCharacterMessage(touchMessages[Math.floor(Math.random() * touchMessages.length)], 'excited');

  useEffect(() => {
    const prods = products || [];
    const discounted = prods.filter(item => item.mrp && Number(item.mrp) > Number(item.price));
    setSaleItems(discounted.length > 0 ? discounted : prods.slice(0, 8)); 
  }, [products]);

  useEffect(() => { setTimeout(() => showCharacterMessage("Welcome to the Sale! Grab a deal, Queen! 🛍️👑", 'happy'), 500); }, []);
  ...
  const getMainImage = (item) => { if (!item || !item.image) return "https://placehold.co/400x400?text=No+Image"; if (Array.isArray(item.image)) return item.image[0]; try { return JSON.parse(item.image)[0]; } catch(e) { return item.image; } };
  ...
  const formatPrice = (price) => { const num = Number(price || 0); const formatted = num.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }); const [whole, decimal] = formatted.split('.'); return { whole, decimal }; };
  const getAllImages = (item) => { if (!item || !item.image) return []; if (Array.isArray(item.image)) return item.image; try { const parsed = JSON.parse(item.image); return Array.isArray(parsed) ? parsed : [item.image]; } catch(e) { return [item.image]; } };

  const openModal = (product) => { setSelectedProduct(product); setActiveImage(getMainImage(product)); setQuantity(1); setTimeout(() => setIsAnimating(true), 10); showCharacterMessage(hoverMessages[Math.floor(Math.random() * hoverMessages.length)], 'happy'); };
  const closeModal = () => { setIsAnimating(false); setTimeout(() => setSelectedProduct(null), 400); };
  const handleAddToCart = (id, qty) => { addToCart(id, qty); showCharacterMessage(addToCartMessages[Math.floor(Math.random() * addToCartMessages.length)], 'love'); closeModal(); };
    ...
    <div className={`sale-page ${selectedProduct ? 'blur-bg' : ''}`}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .sale-page { width: 125%; min-height: 100vh; background: #ffe8f0; font-family: 'Jost', sans-serif; position: relative; margin-left: -13.9%; }
        ...
        .sparkle-1 { top: 10%; left: 10%; animation-delay: 0s; } .sparkle-2 { top: 20%; right: 5%; animation-delay: 0.5s; } .sparkle-3 { bottom: 30%; left: 5%; animation-delay: 1s; } .sparkle-4 { bottom: 20%; right: 10%; animation-delay: 1.5s; }
        ...
        .heart-1 { top: 20%; left: -20px; animation-delay: 0s; } .heart-2 { top: 50%; right: -20px; animation-delay: 1s; } .heart-3 { bottom: 30%; left: -15px; animation-delay: 2s; }
        ...
        /* HERO */
        .sale-hero { padding: 140px 20px 80px; background: linear-gradient(180deg, #ffd6e4 0%, #ffe8f0 60%, #ffe8f0 100%); text-align: center; position: relative; overflow: hidden; }
        ...
        /* ════════════════════════════════════════
           PRODUCT GRID — CORRECTED & BEAUTIFUL
        ════════════════════════════════════════ */
        .sale-grid-section { padding: 70px 5vw 120px; background: #ffe8f0; }

        /* Section heading */
        .grid-header { text-align: center; margin-bottom: 50px; }
        .grid-eyebrow { font-family: 'Jost', sans-serif; font-size: 0.72rem; font-weight: 700; letter-spacing: 5px; text-transform: uppercase; color: #d4af37; display: block; margin-bottom: 10px; }
        .grid-header h2 { font-family: 'Cinzel', serif; font-size: clamp(1.6rem, 3.5vw, 2.6rem); color: #1a1a1a; margin: 0 0 10px; }
        .grid-header p { font-family: 'Jost', sans-serif; color: #aaa; font-size: 0.92rem; margin: 0; }
        ...
        .sale-card {
            ...
            position: relative;
            cursor: pointer;
            transition: transform 0.42s cubic-bezier(0.165,0.84,0.44,1), box-shadow 0.42s ease;
            box-shadow: 0 3px 14px rgba(201,85,122,0.07);
            border: 1px solid rgba(255,200,220,0.45);
        }
        .sale-card:hover {
            transform: translateY(-14px);
            box-shadow: 0 28px 55px rgba(201,85,122,0.15), 0 6px 18px rgba(212,175,55,0.1);
        ...
        .sale-card:hover .card-img-container img { transform: scale(1.1); }

        /* Gradient overlay + Quick View */
        .card-overlay {
        ...
        .sale-card:hover .card-overlay { opacity: 1; }
        .qv-btn {
        ...
        .sale-card:hover .qv-btn { transform: translateY(0); }
        ...
        .discount-badge {
            ...
            font-size: 0.7rem; font-weight: 700; border-radius: 30px;
        ...
        .wishlist-btn:hover { transform: scale(1.18); background: #fff; }

        /* Card body */
        .sale-details { padding: 15px 14px 18px; text-align: center; }
        .sale-details h3 {
            font-family: 'Cinzel', serif; font-size: 0.88rem; font-weight: 600;
            color: #222; margin: 0 0 8px; line-height: 1.35;
        ...
        .price-row { display: flex; justify-content: center; align-items: center; gap: 9px; flex-wrap: wrap; }
        .new-price { color: #c9336a; font-weight: 700; font-size: 1.1rem; font-family: 'Cinzel', serif; display: flex; align-items: flex-start; line-height: 1; }
        .p-rupee { font-size: 0.68rem; margin-top: 2px; }
        .p-whole { font-size: 1.1rem; }
        .p-decimal { font-size: 0.62rem; margin-top: 3px; opacity: 0.75; }
        .old-price { text-decoration: line-through; color: #c8c8c8; font-size: 0.78rem; font-family: 'Jost', sans-serif; }
        ...
        /* RESPONSIVE */
        @media (max-width: 1400px) { .sale-grid { grid-template-columns: repeat(4, 1fr); } }
        @media (max-width: 1200px) { .sale-page { width: 100%; margin-left: 0; } .sale-grid { grid-template-columns: repeat(3, 1fr); gap: 18px; } .card-img-container { height: 240px; } .footer-grid { grid-template-columns: 1fr; gap: 40px; } .footer-bottom { flex-direction: column; gap: 20px; text-align: center; } }
        @media (max-width: 860px) { .sale-grid { grid-template-columns: repeat(2, 1fr); gap: 14px; } .card-img-container { height: 210px; } .sale-hero { padding-top: 120px; } .clean-modal-card { flex-direction: column; width: 95%; height: 90vh; overflow-y: auto; } .clean-modal-left, .clean-modal-right { width: 100%; height: auto; } .permanent-character { bottom: 15px; right: 15px; } .character-container { width: 140px; height: 160px; } .character-body { width: 70px; height: 85px; bottom: 35px; } .character-head { width: 60px; height: 60px; bottom: 105px; } .character-speech { font-size: 0.8rem; padding: 10px 15px; max-width: 180px; top: -80px; } }
        @media (max-width: 480px) { .sale-grid { grid-template-columns: repeat(2, 1fr); gap: 10px; } .card-img-container { height: 165px; } .sale-details { padding: 10px 8px 12px; } .sale-details h3 { font-size: 0.75rem; } .new-price { font-size: 0.9rem; } }
      `}</style>
      ...
      <div className="permanent-character" onClick={handleCharacterClick}>
        <div className={`character-container ${isJumping ? 'jumping' : ''}`}>
          ...
          <div className={`character-sparkles ${sparkleEffect ? 'active' : ''}`}>
            <div className="sparkle sparkle-1">✨</div><div className="sparkle sparkle-2">⭐</div>
            <div className="sparkle sparkle-3">💫</div><div className="sparkle sparkle-4">✨</div>
          </div>
          <div className="floating-hearts">
            <div className="heart heart-1">💕</div><div className="heart heart-2">💖</div><div className="heart heart-3">💗</div>
          </div>
      ...
      <div className="sale-hero">
        ...
        <div className="timer-container">
          <div className="time-box"><span className="time-val">{String(timeLeft.hours).padStart(2,'0')}</span><span className="time-label">Hrs</span></div>
          <div className="time-box"><span className="time-val">{String(timeLeft.minutes).padStart(2,'0')}</span><span className="time-label">Mins</span></div>
          <div className="time-box"><span className="time-val">{String(timeLeft.seconds).padStart(2,'0')}</span><span className="time-label">Secs</span></div>
        </div>
      </div>
      ...
      <section className="sale-grid-section">
        <div className="grid-header">
          <span className="grid-eyebrow">✦ Exclusive Offers ✦</span>
          <h2>Sale Picks</h2>
          <p>Handpicked pieces at unbeatable prices — today only</p>
        </div>
        ...
        <div className="sale-grid">
          {saleItems.length > 0 ? saleItems.map((product, index) => {
            ...
            const oldWhole = product.mrp ? formatPrice(product.mrp).whole : '';
            return (
              <div key={index} className="sale-card" onClick={() => openModal(product)}>
                ...
                <div className="card-img-container">
                  ...
                  <div className="card-overlay"><button className="qv-btn">Quick View</button></div>
                </div>
                <div className="sale-details">
                  ...
                  <div className="price-row">
                    <span className="new-price">
                      <span className="p-rupee">₹</span>
                      <span className="p-whole">{whole}</span>
                      <span className="p-decimal">.{decimal}</span>
                    </span>
                    {product.mrp && Number(product.mrp) > Number(product.price) && (
                      <span className="old-price">₹{oldWhole}</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div style={{textAlign:'center',gridColumn:'1/-1',padding:'80px 20px'}}>
              <div style={{fontSize:'2.5rem',marginBottom:'14px'}}>✨</div>
              <h3 style={{fontFamily:'Cinzel,serif',color:'#c9557a',marginBottom:'6px'}}>Loading Exclusive Offers...</h3>
              <p style={{fontFamily:'Jost,sans-serif',color:'#bbb',fontSize:'0.9rem'}}>Our best deals are on the way!</p>
            </div>
          )}
        </div>
      </section>
      ...
      <footer className="luxury-footer">
        <div className="footer-grid">
          ...
          <div className="footer-col">
            ...
            <div className="footer-links">
              <Link to="/">Home</Link><Link to="/collection">New Arrivals ✨</Link>
              <Link to="/collection">Necklaces</Link><Link to="/collection">Earrings</Link>
            </div>
          </div>
          <div className="footer-col">
            <h3>Information</h3>
            <div className="footer-links">
              ...
              <Link to="/ShippingPolicy">Shipping Policy</Link><Link to="/Policy">Privacy Policy</Link>
            </div>
          </div>
          <div className="footer-col">
        ...
        <div className="footer-bottom">
          ...
          <div className="payment-methods">
            <svg className="payment-icon" viewBox="0 0 38 24"><path d="M35 0H3C1.3 0 0 1.3 0 3v18c0 1.7 1.4 3 3 3h32c1.7 0 3-1.3 3-3V3c0-1.7-1.4-3-3-3z" fillOpacity="0"/><path d="M11.875 16.094l1.85-11.45h2.95l-1.85 11.45h-2.95z"/></svg>
            ...
            <div className="clean-modal-right">
              ...
              <div style={{display:'flex', alignItems:'center', gap:'14px', marginBottom:'24px'}}>
                <div className="clean-price">
                  <span style={{fontSize:'1rem',marginTop:'5px'}}>₹</span>
                  <span>{formatPrice(selectedProduct.price).whole}</span>
                  <span style={{fontSize:'1rem',marginTop:'5px'}}>.{formatPrice(selectedProduct.price).decimal}</span>
                </div>
                {selectedProduct.mrp && Number(selectedProduct.mrp) > Number(selectedProduct.price) && (
                  <span style={{fontSize:'0.95rem',textDecoration:'line-through',color:'#bbb',fontFamily:'Jost,sans-serif'}}>₹{formatPrice(selectedProduct.mrp).whole}</span>
                )}
              </div>
